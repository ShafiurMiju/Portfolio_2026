# My Portfolio

A stunning, modern portfolio website built with Next.js featuring animated backgrounds, interactive elements, and a gorgeous dark theme.

## ✨ Features

- 🌈 **Animated gradient background** with floating blobs that react to mouse movement
- ✨ **Custom cursor follower** with blend modes
- 🎨 **Dark cyberpunk theme** with purple, cyan, and pink gradients
- 💫 **Smooth animations** and transitions throughout
- 📊 **Interactive skill bars** with progress animations
- 🎯 **Beautiful project showcases** with hover effects
- 🔗 **Stylish social media cards**
- 📱 **Fully responsive** design for all devices

## 🚀 Tech Stack

- **Next.js 14** - React framework
- **React Hooks** - State management
- **CSS Modules** - Scoped styling
- **Google Fonts** - Inter & Space Grotesk

## 🛠️ Getting Started

1. Clone the repository:
\`\`\`bash
git clone https://github.com/ShafiurMiju/myPortfolio.git
cd myPortfolio
\`\`\`

2. Install dependencies:
\`\`\`bash
npm install
\`\`\`

3. Run the development server:
\`\`\`bash
npm run dev
\`\`\`

4. Open [http://localhost:3000](http://localhost:3000) in your browser

## 📝 Customization

Update the following in \`pages/index.js\`:
- Your name and title in the hero section
- About section content
- Skills and expertise levels
- Project details, descriptions, and links
- Contact information and social media handles

## 📄 License

MIT License - feel free to use this portfolio for your own projects!

## 👤 Author

**Shafiur Miju**

---

Built with passion and lots of ☕
